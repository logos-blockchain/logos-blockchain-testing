...
```rust
let mut scenario = AppHost::scenario()
    .with_app_using(JobStackContainerApp::new(), ComposeProvisioner::default())
    .with_workload(EnqueueJobs::new(10))
    .with_expectation(AllJobsCompleted::new(10))
    .build()?;
```
...
`ComposeProvisioner` share one project, so the services remain mutually
...
- `ClusterApp` manages a uniform cluster through the active cluster
  provisioner and exposes a `ClusterHandle`.
- `ComposeProvisioner` manages container services in one Compose project.
...
Attached and external sources are part of the same cluster request:
`ClusterApp::with_external_nodes` joins external endpoints to a managed
cluster, and `ClusterRequest::attached`/`::external` describe non-managed
sources. Node control and cluster readiness arrive on the resulting
`ClusterHandle` when the backend grants them; the app layer does not create
replacements.
...
A single uniform cluster is the degenerate composition: one
`ClusterApp` under `AppHost::scenario()`. For composed systems, prefer this
app-layer shape instead of adding app-specific code to TF.
