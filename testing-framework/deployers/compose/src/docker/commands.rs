...
/// Runs `docker compose up -d --no-deps <services...>` for a set of services.
pub async fn compose_up_services(
    compose_path: &Path,
    project_name: &str,
    root: &Path,
    services: &[String],
    ...
    info!(
        ...
        root = %root.display(),
        ?services,
        "running docker compose up for selected services"
    );
