...
use crate::{
    descriptor::{EnvEntry, NodeDescriptor},
    ...
    lifecycle::cleanup::{RunnerCleanup, preserve_requested},
    session::{
        ClusterKey, ClusterServices, ComposeProvisioner, ComposeSession, ComposeSessionCleanup,
        RunnerPorts, SessionPreservation, session_descriptor,
    },
};
...
fn no_clusters() -> BTreeMap<ClusterKey, ClusterServices> {
    BTreeMap::new()
}
...
impl ComposeProvisioner {
    async fn start_session(
        ...
        let runner_ports = {
            ...
            loop {
                ...
                let descriptor = match session_descriptor(
                    &no_clusters(),
                    request.services(),
                    reservation.ports(),
                ) {
        ...
        self.store_session(ComposeSession {
            project,
    ...
    async fn extend_session(
        ...
        ensure_names_available(session.service_names(), request.services())?;
        ...
        for attempt in 1..=PORT_BIND_ATTEMPTS {
            ...
            let descriptor = session_descriptor(&session.clusters, &services, &runner_ports)?;
    ...
    async fn rollback_or_poison(
        ...
        if extension_rollback_preserves(previous, retrying) {
    ...
    fn poison_session(
        ...
        match slot.as_mut() {
            ...
            Some(session) => warn!(
                ...
                "stale compose stack poison request; leaving the live session untouched"
            ),
            None => {}
        }
    }
...
/// Applies the shared preservation policy to a failed stack extension: the
/// environment or any session participant may request that services stay
/// running instead of being rolled back.
fn extension_rollback_preserves(previous: &ComposeSession, retrying: bool) -> bool {
    !retrying && (preserve_requested() || previous.preserve.requested())
}
...
mod tests {
    ...
    use super::{
        ensure_names_available, extension_rollback_preserves, no_clusters, remove_service_files,
        reserve_runner_ports, validate_request, write_service_files,
    };
    use crate::{
        infrastructure::{project::ComposeProject, template::write_compose_file},
        session::{ComposeSession, RunnerPorts, SessionPreservation, session_descriptor},
    };
    ...
    fn extension_rejects_a_name_already_in_the_session() {
        assert!(ensure_names_available(["worker"], &[service("worker")]).is_err());
    }

    #[test]
    fn extension_rejects_a_name_held_by_a_cluster_service() {
        assert!(ensure_names_available(["node-0"], &[service("node-0")]).is_err());
        assert!(ensure_names_available(["node-0"], &[service("worker")]).is_ok());
    }
    ...
    fn published_ports_are_explicit_and_distinct() {
        ...
        let descriptor = session_descriptor(&no_clusters(), &services, ports).unwrap();
    ...
    fn environment_values_render_as_literal_compose_data() {
        ...
        let reservation = reserve_runner_ports(&services).unwrap();
        let descriptor =
            session_descriptor(&no_clusters(), &services, reservation.ports()).unwrap();
    ...
    #[test]
    fn session_preserve_requests_stop_stack_extension_rollbacks() {
        let session = ComposeSession {
            project: ComposeProject::new(
                std::path::PathBuf::from("/tmp/compose.yml"),
                "stack-session",
        ...
        assert!(!extension_rollback_preserves(&session, false));
        assert!(!extension_rollback_preserves(&session, true));

        session.preserve.request();

        assert!(extension_rollback_preserves(&session, false));
        assert!(
            !extension_rollback_preserves(&session, true),
            "retries must still roll back so the next attempt starts clean"
        );
    }
