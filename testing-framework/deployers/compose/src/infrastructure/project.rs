...
use crate::{
    docker::{
        commands::{
            ...
            compose_up_services, dump_compose_logs,
        },
...
impl ComposeProject {
    ...
    pub(crate) fn new(compose_file: PathBuf, name: impl Into<String>, root: PathBuf) -> Self {
        Self {
            compose_file,
            name: name.into(),
            root,
    ...
    pub(crate) async fn up_services_unlocked(
        &self,
        services: &[String],
    ) -> Result<(), ComposeCommandError> {
        compose_up_services(&self.compose_file, &self.name, &self.root, services).await
    }
