...
pub enum ComposeRunnerError {
    ...
    OnDemandUnsupported,
    #[error("compose session is unavailable after a failed extension")]
    SessionPoisoned,
    #[error(
    ...
    #[error("compose cluster extension failed: {failure}; rollback also failed: {rollback}")]
    ExtensionRollback {
        failure: Box<ComposeRunnerError>,
        rollback: Box<ComposeRunnerError>,
    },
